...
def nearest_power_of_two(x: float, xmax: float) -> int:
    return min(max(2 ** int(round(math.log2(x))), 1), xmax)
...
class T5Attention(nn.Module):
    def __init__(
        ...
        self.base_n_heads = config.base_n_heads
        self.base_tilde_d = config.base_tilde_d
        ...
        if is_cross_attention:
            effective_k = self.encoder_num_layers + layer_idx + 1
        else:
            effective_k = layer_idx + 1
        self.n_heads = nearest_power_of_two(self.base_n_heads * (effective_k ** self.alpha), self.base_n_heads)
        self.d_kv = nearest_power_of_two(self.base_tilde_d * (effective_k ** self.beta), self.d_model // self.n_heads)
        self.key_value_proj_dim = self.d_kv
...
class T5LayerSelfAttention(nn.Module):
    def __init__(self, config, is_decoder, has_relative_attention_bias=False, layer_idx: Optional[int] = None):
        super().__init__()
        self.SelfAttention = T5Attention(
            config, has_relative_attention_bias=has_relative_attention_bias, layer_idx=layer_idx, is_decoder=is_decoder, is_cross_attention=False
        )
        self.layer_norm = T5LayerNorm(config.d_model, eps=config.layer_norm_epsilon)
        self.dropout = nn.Dropout(config.dropout_rate)
...
class T5LayerCrossAttention(nn.Module):
    def __init__(self, config, is_decoder, layer_idx: Optional[int] = None):
        super().__init__()
        self.EncDecAttention = T5Attention(config, has_relative_attention_bias=False, layer_idx=layer_idx, is_decoder=is_decoder, is_cross_attention=True)
        self.layer_norm = T5LayerNorm(config.d_model, eps=config.layer_norm_epsilon)
        self.dropout = nn.Dropout(config.dropout_rate)
...
class T5Block(nn.Module):
    def __init__(self, config, has_relative_attention_bias=False, layer_idx: Optional[int] = None):
        ...
        self.layer.append(
            T5LayerSelfAttention(config, has_relative_attention_bias=has_relative_attention_bias, is_decoder=self.is_decoder, layer_idx=layer_idx)
        )
        if self.is_decoder:
            self.layer.append(T5LayerCrossAttention(config, layer_idx=layer_idx, is_decoder=self.is_decoder,))
